...
export default function ExplosivesPage() {
  ...
  const handleSaveTransaction = async () => {
    ...
    if (selectedItem && selectedItem._id === payload.itemId) {
      ...
      const latestItem = items.find((i) => i._id === payload.itemId);
      if (latestItem) setSelectedItem(latestItem);
    }
  ...
  const summary = useMemo(() => {
    ...
    const totalPurchased = itemTransactions
      .filter((t) => t.type === "purchase")
      .reduce((sum, t) => sum + Number(t.quantity), 0);

    const totalUsed = itemTransactions
      .filter((t) => t.type === "usage")
      .reduce((sum, t) => sum + Number(t.quantity), 0);

    const totalPurchaseCost = itemTransactions
      .filter((t) => t.type === "purchase")
      .reduce((sum, t) => sum + Number(t.cost || 0), 0);

    const totalUsageCost = itemTransactions
      .filter((t) => t.type === "usage")
      .reduce((sum, t) => sum + Number(t.cost || 0), 0);

    const netAdjustments = itemTransactions
  ...
  if (isLoading) {
    return (
      <Box sx={{ p: 4 }}>
        <Typography>Loading Explosives Inventory...</Typography>
      </Box>
    );
  }
    ...
    <Box sx={{ p: { xs: 2, md: 4 }, pb: 10 }}>
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Track stock, usage, alerts and transaction history
          </Typography>
        </Box>
          ...
          return (
            <Grid item xs={12} sm={6} md={4} key={item._id}>
              <Card
                sx={{
                  cursor: "pointer",
                  border: isLow ? "1px solid" : "1px solid transparent",
              ...
              >
                <CardContent>
                  ...
                  <Typography variant="h4" sx={{ fontWeight: "bold", color: isLow ? "error.main" : "primary.main" }}>
      ...
      <Dialog open={openItemDialog} onClose={handleCloseItemDialog} fullWidth maxWidth="sm">
        ...
        <DialogContent dividers sx={{ display: "flex", flexDirection: "column", gap: 2.5, pt: 2 }}>
          ...
          {editingId && (
            <Alert severity="info">
              Stock cannot be edited directly. Use transactions to update stock.
            </Alert>
          )}
        </DialogContent>
      ...
      <Dialog open={openTransactionDialog} onClose={() => setOpenTransactionDialog(false)} fullWidth maxWidth="sm">
        ...
        <DialogContent dividers sx={{ display: "flex", flexDirection: "column", gap: 2.5, pt: 2 }}>
          ...
          <Alert severity={transactionForm.type === "usage" ? "warning" : "info"}>
      ...
      <Dialog open={!!selectedItem} onClose={() => setSelectedItem(null)} fullWidth maxWidth="md">
        ...
        <DialogContent dividers>
          <Box sx={{ mb: 3 }}>
            ...
            <Stack direction="row" spacing={1} mt={2}>
              <Chip label={selectedItem?.category} />
              <Chip
